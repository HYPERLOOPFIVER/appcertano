...
export default function Home() {
  ...
  // Fetch stories from following users
      ...
      for (const uid of userIds) {
        ...
        if (userStories.length > 0) {
          ...
          allStories.push({
            userId: uid,
            stories: userStories,
            hasUnseenStories: userStories.some(s => !s.viewers?.includes(userId))
          });
        }
      }
    ...
    return (
      <View style={styles.storiesSection}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storiesContainer}>
          ...
          >
            <View style={[styles.storyAvatarContainer, styles.addStoryContainer]}>
              <Ionicons name="add" size={28} color="#006D77" />
            </View>
          ...
          {stories.map((userStory) => {
            const userName = userNames[userStory.userId] || 'User';
            const avatar = userAvatars[userStory.userId];
            const isOwnStory = userStory.userId === currentUserId;
            
            return (
              <TouchableOpacity 
                key={userStory.userId}
                style={styles.storyItem}
                onPress={() => router.push(`/story/${userStory.userId}`)}
              >
                <View style={[
                  styles.storyAvatarContainer, 
                  { width: 64, height: 64, borderRadius: 32 },
                  userStory.hasUnseenStories && styles.storyAvatarUnseen
                ]}>
                  {avatar ? (
                    <Image 
                      source={{ uri: avatar }} 
                      style={[styles.storyAvatarImage, { width: 58, height: 58, borderRadius: 29 }]} 
                    />
                  ) : (
                    <Text style={[styles.storyAvatarText, { fontSize: 24 }]}>
                      {userName.charAt(0).toUpperCase()}
                    </Text>
                  )}
                </View>
                <Text style={styles.storyUserName} numberOfLines={1}>
                  {isOwnStory ? 'You' : userName.split(' ')[0]}
                </Text>
              </TouchableOpacity>
            );
          })}
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRight}>
          <TouchableOpacity style={styles.headerIcon} onPress={() => router.push('/notifications')}>
            <Ionicons name="notifications-outline" size={24} color="#006D77" />
          </TouchableOpacity>
